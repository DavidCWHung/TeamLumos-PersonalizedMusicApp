...
fun PlayerScreen(
    ...
    if (state.videos.isNotEmpty()){
        ...
        if (isLandscape)
            ScreenWithFavListLandscape(state, onEvent) // Render when the oritentation is landscape
        else
            ScreenWithFavList(state, onEvent) // Render when the orientation is portrait or undefined
    } else {
        ScreenWithoutFavList() // Render when the favourites list is empty
    }
}

@Composable
fun ScreenWithoutFavList(
){
    Column (
...
@RequiresApi(Build.VERSION_CODES.O)
@Composable
fun ScreenWithFavListLandscape(state: VideoState, onEvent: (VideoEvent) -> Unit)
{
    // The index of the current video
    var idx by remember {
        mutableStateOf(0)
    ...
    // This counter will change the boolean to true in every 1/10s
    object : CountDownTimer(100, 100) {
